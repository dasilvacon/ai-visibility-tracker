...
def display_html_report():
    ...
    if st.session_state.get('role') == 'admin' and st.session_state.brand_name is None:
        # Get clients from ClientRegistry (source of truth - synced from GCS on startup)
        available_brands = []
        ...
        try:
            ...
            available_brands = sorted([c['name'] for c in all_clients])
            ...
            try:
                ...
                if clients_file.exists():
                    ...
                    available_brands = sorted([c['name'] for c in clients_data.get('clients', [])])
            except Exception:
                pass
